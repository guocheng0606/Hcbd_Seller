...
public class PrintUtil {
    ...
    public static void printTest(BluetoothSocket bluetoothSocket, List<OrderItemInfo> list) {
        try {
            PrintUtil pUtil = new PrintUtil(bluetoothSocket.getOutputStream(), "GBK");
            // 店铺名 居中 放大
            pUtil.printAlignment(1);
            pUtil.printLargeText(list.get(0).getItem().getMenu().getType().getName());
            pUtil.printLine();
            pUtil.printAlignment(0);
            pUtil.printLine();

            pUtil.printTwoColumn("桌号：", list.get(0).getItem().getCar().getTabCode()+"_"+list.get(0).getItem().getCar().getSeq());
            pUtil.printLine();

            pUtil.printTwoColumn("时间：", list.get(0).getItem().getCar().getCreateTime().replace("T", " "));
            pUtil.printLine();
            ...
            for (OrderItemInfo info : list) {
                if (info.getMenu().getCode().indexOf("P") != -1){
                    pUtil.printThreeColumn(info.getMenu().getName()+"（"+info.getItem().getPkg()+"）", "X"+info.getItem().getNum(), "￥"+ CommonUtils.subZeroAndDot(String.valueOf(info.getItem().getAmt())));
                } else {
                    pUtil.printThreeColumn(info.getMenu().getName(), "X"+info.getItem().getNum(), "￥"+ CommonUtils.subZeroAndDot(String.valueOf(info.getItem().getAmt())));
                }
            ...
            pUtil.printTwoColumn("合计:", "￥"+ CommonUtils.subZeroAndDot(String.valueOf(list.get(0).getItem().getCar().getAmt())));
            pUtil.printLine();

            pUtil.printTwoColumn("应付:", "￥"+ CommonUtils.subZeroAndDot(String.valueOf(list.get(0).getItem().getCar().getAmt())));
            pUtil.printLine();
            ...
            pUtil.printAlignment(0);
            pUtil.printLine();
            pUtil.printText("谢谢惠顾，欢迎再次光临！");
            pUtil.printLine();
            pUtil.printText("www.whhcbd.com");
            pUtil.printLine();
