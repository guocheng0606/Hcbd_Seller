...
public abstract class BasePrintActivity extends AppCompatActivity {
     ...
     * 蓝牙状态发生变化时回调
     */
    public abstract void onBluetoothStateChanged(Intent intent);
    ...
    public void connectDevice(BluetoothDevice device) {
        if (checkBluetoothState() && device != null) {
            mConnectTask = new ConnectBluetoothTask().execute(device);
        }
    }
    ...
    class ConnectBluetoothTask extends AsyncTask<BluetoothDevice, Integer, BluetoothSocket> {
        ...
        protected BluetoothSocket doInBackground(BluetoothDevice... params) {
            ...
            if (mSocket == null) {
                MyApplication.getInstance().setmBluetoothDevice(null);
                MyApplication.getInstance().setmBluetoothSocket(null);
            } else {
                if (mSocket.isConnected()){
                    MyApplication.getInstance().setmBluetoothDevice(params[0]);
                    MyApplication.getInstance().setmBluetoothSocket(mSocket);
                }
            }
            //onConnected(mSocket);
            return mSocket;
